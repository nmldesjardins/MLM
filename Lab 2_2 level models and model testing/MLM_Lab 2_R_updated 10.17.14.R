...
# Here's an alternate way, that doesn't require writing a function - it just uses the anova function that's already built in (thanks, Joe!)
# You will get slightly different results for lme4 models because anova will re-fit the model using ML instead of REML
# Although there is debate about whether this is necessary, it's actually a better way to test models that differ in their fixed effects (see Kreft & de Leeuw, 1998)
...
### OPTION 4: FIX THE COVARIANCES TO ZERO.
# Hurray for John, who figured this out in nlme!
...
## I'd reccomend only doing this in nlme.
## If you want to do it in lme4, you need to create a new variable to represent the interaction, AND you can only have continuous variables.

# Kludge: the lme4 version - DONT USE THIS.
# create the interaction term

atlas$useXse<-(as.numeric(atlas$use0)*atlas$SE0)

# specify the model with separate random effects for each variable
# "as.numeric" tells R to treat use as a continous (rather than catergorical) variable. 
# If this isn't specified, lme4 will estimate separate random effects + the covariance between them for each level of the categorical variable.

model10<-lmer(coachtol0~as.numeric(use0)+SE0+useXse+(1|schoolid)+(0+SE0|schoolid)+(0+as.numeric(use0)|schoolid)+(0+useXse|schoolid), data=atlas)


# Much better: the nlme version - USE THIS.
# nlme lets you specify specific covariance matrices; for more information see ?pdMat and ?pdClasses
# here, we're telling it to estimate a diagnoal matrix (pdDiag) in which the variances (which are along the diagonal in the matrix) are freely estimated
# but the covariances (the off-diagonal values) are fixed to zero.
# the "form = ~use*SE0" tells it that we want a random intercept and random slopes for use0, SE0, and use0*SE0
# if you wanted fewer random effects (like Option 2, above), but still didn't want the covariances, you would just change the formula that follows "form =" to reflect what you want.

model10n<-lme(coachtol0~use0*SE0,random=list(schoolid=pdDiag(form=~use0*SE0)), data=atlas, na.action = na.exclude)
summary(model10n)
