...
## How did I find out that gee() didn't like value labels? ##

# A lot of debugging, following the instructions here: http://www.r-bloggers.com/tracking-down-errors-in-r/
# some useful things:
traceback() # helps elucidate error messages
options(warn=2) # turns warnings into errors, and so stops a function when the thing causing the error is encountered
debug() # shows how the sausage is made. this lets you run the function line by line so you can see where exactly the error occurs.
