...
### blah blah blah things for nicole ###

# unconditional quadratic growth: 2 level 
modL22<-lme(attitude~year_c+year_c2, random=~ year_c+year_c2|respondant, na.action=na.exclude, data=data, control=list(opt="optim"))
summary(modL22)
...
# test the difference between models with and without the random effect of year^2
# note that we're only chaning the random effects, so the deviance test with REML estimation is 100% kosher.
anova(mod4,mod5)


# since the simpler model doesn't fit the data any worse than the more complex one, that's what we'll use to proceed.


# specify autoregressive lag-1 L1 structure:
# note that the data MUST be ordered by time for each individual for this to work properly
...
mod.homo<-lme(attitude~year_c+year_c2+religpref, random=~ 1|respondant, na.action=na.exclude, data=data, control=list(opt="optim")) # this one

mod.homo2<-lme(attitude~year_c+year_c2+religpref, random=~ 1|respondant, correlation = corCompSymm, na.action=na.exclude, data=data, control=list(opt="optim")) # almost identical to the default, but has 1 more df (is estimating rho/a single covariance)
...
mod.hetero2<-lme(attitude~year_c+year_c2+religpref, random=~ 1|respondant, weights=varIdent(form=~1|year_c), na.action=na.exclude, data=data, control=list(opt="optim")) # this one
